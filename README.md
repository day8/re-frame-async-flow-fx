...
Read that last paragraph again.  It distills further to: when event E1 happens then `dispatch` event E2.  Or, simply, When-E1-Then-E2. 
...
Because we can't issue a database query until we have a database connection, the 1st rule (above) says:
...
A `rule` is a map with the following 3 fields:
  ...
  - `:when`  one of `:seen?`, `:seen-both?`. `:seen-all-of?`, `:seen-any-of?`  
    `:seen?`, `:seen-both?` and `:seen-all-of?` are effectively the same.
...
Notes:
  1.  This pattern is very flexible. You could modify it to handle more complex FSM.
  2.  All the work is done in a normal event handler (created for you).  And it is all organised around events. 
      So this solution is based on re-frame fundamentals. 
...
First, the state machine is encoded in javascript "code" (the generator function becomes 
the state machine). In clojure, we have a preference for "programming in data"were possible.  
...
we have core.async instead of generator functions. That is left as an exercise for the modivated reader.
